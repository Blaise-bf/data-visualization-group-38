    ...
    const df = await FileAttachment("data-2.csv").csv({typed: true});
    
     
    const data = df.columns.slice(1).flatMap((age) => data.map((d) => ({state: d.State, age, population: d[age]})));
    console.log(data)
    const  width = 700;
    const height = width;
    ...
    const svg = d3.select("#demoradial")
        .append("svg")
        .attr("width", width)
        .attr("height", height)
