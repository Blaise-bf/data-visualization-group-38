...
```{r}
revenue_by_type <- df_chart1 %>%
  group_by(business_unit, month, year, type) %>%
  summarize(
    revenue = sum(revenue, na.rm = TRUE),
    type_avg_revenue = mean(revenue, na.rm = TRUE),
    total_type_quantity = sum(quantity, na.rm = TRUE), .groups = "drop"
...
df_month <- revenue_by_type |> 
  left_join(revenue_by_month, join_by(business_unit, year, month)) |> 
...
write(json_data_month, "monthly_revenue.json")
```
...
```{r}
df_delays <- df_join |> 
  group_by(year, brand_name, month_name) |> 
