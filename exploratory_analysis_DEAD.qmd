...
```{r}
# Ensures every year has 52 weeks
standardize_week <- function(date) {
  wk <- lubridate::week(date)
  # Replace week number 53 with 52
  ifelse(wk > 52, 52, wk)
}
```
...
df_join <- df_orders_wide |> 
  ...
  mutate(
    ...
    week = standardize_week(order_date),
...
```{r}
df_join |> 
  mutate(is_week = isoweek(order_date), .before =  1) |> 
  select(is_week) |> 
  distinct()
```
