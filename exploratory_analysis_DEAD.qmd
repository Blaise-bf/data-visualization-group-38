...
#| eval: false
#| echo: false
df_products <- orders |> separate_rows(products, sep = ", ") 
df_price <- orders |> separate_rows(product_prices_in_cp, sep = ", ")
```
...
df_join <- orders |> 
  ...
  mutate(gold = parse_number(gold),
         silver = parse_number(silver),
         copper = parse_number(copper)) |> 
  inner_join(customers |> 
               rename(customer_id = account_code), by = 'customer_id') |> 
  inner_join(regions, by = "territory")
